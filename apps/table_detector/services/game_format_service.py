...
class GameFormatService:
    ...
    def game_to_dict(window_name: str, game: GameSnapshot) -> dict:
        ...
        return {
            ...
            'solver_link': FlopHeroLinkService.generate_link(game)
        }
    ...
    @staticmethod
    def _get_positions_for_web(positions) -> List[Dict]:
        if not positions:
            return []

        formatted = []
        for player_num, position in enumerate(positions.values(), 1):
            formatted.append({
                'player': player_num,
