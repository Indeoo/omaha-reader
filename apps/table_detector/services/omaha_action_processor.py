...
def group_moves_by_street(player_moves: Dict[Position, List[MoveType]]) -> Dict[Street, List[Tuple[Position, MoveType]]]:
    ...
    # Phase 1: Build street-aware chronological action sequence with contextual interpretation
    all_actions = _build_street_aware_action_sequence(player_moves)

    if not all_actions:
        return {Street.PREFLOP: [], Street.FLOP: [], Street.TURN: [], Street.RIVER: []}

    # Phase 2: Use OmahaGame state machine to process actions
    ...
    for position in position_order:
        if action_indices[position] < len(player_moves[position]):
            ...
            interpreted_move = _interpret_action_contextually(
                original_move, actions_this_round, betting_occurred, is_preflop=True
            )
    ...
    for _ in range(max_actions_per_player):
        ...
        for position in position_order:
            if position in player_moves and action_indices[position] < len(player_moves[position]):
                ...
                interpreted_move = _interpret_action_contextually(
                    original_move, actions_this_round, betting_occurred, is_preflop
                )
...
def _process_flop_street_specially(player_moves: Dict[Position, List[MoveType]],
    ...
    for position in position_order:
        if position in player_moves and action_indices[position] < len(player_moves[position]):
            ...
            interpreted_move = _interpret_action_contextually(
                original_move, [], betting_occurred, is_preflop=False
            )
...
def _interpret_action_contextually(
        move: MoveType,
